/* ============================================================
   Table SCF — 36-month projection (M-12 … M+23)
   ============================================================ */
...
window.PROXIMA.ProjectionTable = function ProjectionTable({ sim }) {
  ...
  const cols = [
    ["Mese",            (r) => null,                ""],
    ["Fase",            (r) => null,                ""],
    ["Iscrizioni Check-up", (r) => fmtN(r.totalSignups), ""],
    ["Utenti gratuiti", (r) => fmtN(r.freeUsers),   ""],
    ["Nuovi abbonati",  (r) => fmtN(r.newPaying),   ""],
    ["Persi (churn)",   (r) => fmtN(r.churnedClients), ""],
    ["App",             (r) => fmtN(r.clientsApp),  ""],
    ["Monitor",         (r) => fmtN(r.clientsMonitor), ""],
    ["Live",            (r) => fmtN(r.clientsLive), ""],
    ["Abbonati attivi", (r) => fmtN(r.totalClients), "bold"],
    ["AUM monit. (€k)", (r) => fmtN(Math.round(r.totalAUM/1000)), ""],
    ["Costi costituz.", (r) => fmtE(r.constitutionCosts), ""],
    ["Costi operativi", (r) => fmtE(r.operatingCosts), ""],
    ["App e IA",        (r) => fmtE(r.appCosts), ""],
    ...
    ["Slot Live %",     (r) => Math.round(r.liveUtilization*100)+"%", "util"],
  ...
  const exportCSV = () => {
    const hdr = ["Mese","Fase","Iscrizioni Check-up","Utenti gratuiti","Nuovi abbonati",
      "Churn","App","Monitor","Live","Abbonati attivi","AUM monitorato",
      "Costi costituzione","Costi operativi","App e IA","Personale","Marketing",
      "Costi totali","MRR","Burn netto","Cash","Team","Slot Live %","Budget ads %"];
    const rows = results.map((r) => [
      r.monthLabel, r.phaseName, Math.round(r.totalSignups), Math.round(r.freeUsers),
      Math.round(r.newPaying), Math.round(r.churnedClients),
      Math.round(r.clientsApp), Math.round(r.clientsMonitor), Math.round(r.clientsLive),
      Math.round(r.totalClients), Math.round(r.totalAUM),
      Math.round(r.constitutionCosts), Math.round(r.operatingCosts), Math.round(r.appCosts),
      ...
      Math.round(r.liveUtilization*100), Math.round(r.budgetFactor*100)
    ].join(";"));
    download(new Blob([hdr.join(";") + "\n" + rows.join("\n")], { type: "text/csv;charset=utf-8" }),
      "proxima-scf-proiezione-36m.csv");
    flash("CSV scaricato");
  };

  const exportJSON = () => {
    download(new Blob([JSON.stringify(results, null, 2)], { type: "application/json" }),
      "proxima-scf-proiezione-36m.json");
  ...
  const tdStyle = (flag, r) => {
    ...
    if (flag === "util") return { color: r.liveUtilization > 0.85 ? "#F59E0B" : "#8B98B0" };
    ...
    React.createElement("div", { className: "section" },
      ...
      React.createElement("div", { style: { marginTop: 12, display: "flex", gap: 16, flexWrap: "wrap", fontSize: 11 }, className: "text-dim" },
        React.createElement("span", null,
          ...
          "Riga arancione = slot Live saturi"
        ),
        React.createElement("span", null,
