...
window.PROXIMA.simulate = function (p, months) {
  ...
  for (var m = 1; m <= months; m++) {
    ...
    if (m >= 13) {
      managedAUM *= (1 + sp500Monthly);
      var prevClients = totalClients - newCapped + churn - waitingListBoost;
      if (prevClients > 0 && churn > 0) {
        managedAUM -= managedAUM * (churn / prevClients);
      }
      managedAUM += (newCapped + waitingListBoost) * p.aum.avgPerClient;
    }
    var totalAUM = managedAUM;
