...
| T22 | **Fonti ufficiali non-US → [P]** | Oggi i non-US usano TradingView (snapshot, [U]). Per tag [P] e serie multi-anno servono le fonti ufficiali per-paese: EDINET (JP, API v2 chiave gratuita), DART/OpenDART (KR), MOPS (TW), ESEF via filings.xbrl.org (UE). Grande lavoro, un paese alla volta. Fino ad allora: TradingView + yfinance cross-validati → [V] sul VPS. |
