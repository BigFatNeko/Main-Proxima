...
| T22 | **UE via ESEF → [P]** ✅ FATTO (nomi coperti) | Cablato `fetch_esef.py`: ticker→LEI (GLEIF)→depositi ESEF (filings.xbrl.org)→fatti IFRS multi-anno + mercato TradingView. ENI/ASML: CS-S 38→79 / 37→81. Vedi calibrazione §9. |
| T22b | **ESEF — ampliare copertura** | filings.xbrl.org indicizza bene IT/NL, meno DE/FR (SAP, LVMH: 0 depositi). Ampliare: (a) risoluzione LEI robusta via GLEIF fuzzy per qualsiasi ticker; (b) fallback agli OAM nazionali (Bundesanzeiger DE, AMF FR) dove filings.xbrl.org non copre. Fino ad allora quei nomi → TradingView [U]. |
| T24 | **EDINET/DART/MOPS (JP/KR/TW) → [P]** | Fonti ufficiali asiatiche. EDINET (JP) e DART (KR) richiedono una **chiave API gratuita** che il committente deve registrare; MOPS (TW) è pubblico ma in cinese. Fino ad allora JP/KR/TW → TradingView [U] snapshot. |
| T23 | **yfinance↔TradingView cross-validation live** | Generalizzare `fetchers.cross_validate` a N aggregatori: yfinance ∩ TradingView concordi → [V]. Non testabile qui (yfinance 429); attivo sul VPS. È il cuore del DIP multi-fonte richiesto dal committente. |
